...
import { idlFactory } from "./candid/peerloan_backend.did.js";
...
export interface PeerLoanBackend {
...
// Initialize actor
let peerLoanActor: ActorSubclass<PeerLoanBackend> | null = null;
...
// Get actor instance
export const getPeerLoanActor = async (): Promise<ActorSubclass<PeerLoanBackend>> => {
  if (!peerLoanActor) {
    const agent = await createAgent();
    peerLoanActor = Actor.createActor<PeerLoanBackend>(idlFactory, {
      agent,
      canisterId: BACKEND_CANISTER_ID
    });
  }
  return peerLoanActor;
};
